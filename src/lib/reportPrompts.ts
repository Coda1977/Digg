...
/**
 * Build a prompt for interview summarization
 * Can be customized per template if needed
 */
export function buildInterviewSummaryPrompt(options?: {
  templateName?: string;
  customInstructions?: string;
}): string {
  let prompt = INTERVIEW_SUMMARY_PROMPT;

  if (options?.templateName) {
    prompt += `\n\nContext: This is a "${options.templateName}" interview.`;
  }
...
/**
 * Build a prompt for project analysis
 * Can be customized per template if needed
 */
export function buildProjectAnalysisPrompt(options?: {
  ...
  let prompt = PROJECT_ANALYSIS_PROMPT;

  if (options?.templateName) {
    prompt += `\n\nContext: This is a "${options.templateName}" project.`;
  }
